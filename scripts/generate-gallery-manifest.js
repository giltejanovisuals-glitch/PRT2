/*
 * Build-time image manifest generator for the Editorial & Layout Design
 * gallery (pages/editorial-layout.html).
 *
 * Scans assets/images/gallery/editorial-layout/ for supported image files,
 * reads each file's own header bytes to determine its pixel width/height
 * (no image-processing dependency), and writes js/gallery-editorial-manifest.js
 * as a plain `window.GALLERY_EDITORIAL_MANIFEST = [...]` assignment so it
 * loads with a normal <script> tag like the rest of this site's data files.
 *
...
const SOURCE_DIR = path.join(__dirname, "..", "assets", "images", "gallery", "editorial-layout");
...
function build() {
  ...
  files.forEach((file) => {
    ...
    try {
      ...
      console.warn(`[gallery-manifest] Could not read "${file}": ${error.message}`);
    }
    if (!size || !size.width || !size.height) {
      console.warn(`[gallery-manifest] Skipping "${file}" — could not determine its dimensions.`);
      return;
    }
    manifest.push({
      file,
  ...
  const output = `${header}\nwindow.GALLERY_EDITORIAL_MANIFEST = ${JSON.stringify(manifest, null, 2)};\n`;
  fs.writeFileSync(OUTPUT_FILE, output);
  console.log(
    `[gallery-manifest] Wrote ${manifest.length} image${manifest.length === 1 ? "" : "s"} to ${path.relative(process.cwd(), OUTPUT_FILE)}`
  );
}
